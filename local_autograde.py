# Local Autograder Script -- used to run tests from GitHub Classroom's autograder 
functionality
#  without expending action minutes. 
#
# Version History
# v1.0 - 2022-09-21 - first complete, running version
# v1.1 - 2022-09-22 - change tests to feed both cout and cerr to the same output so
that 
# v1.2 - 2022-09-23 - Correct incorrect path bug when attempting to fix malformed 
scripts. Scripts were getting incorrectly corrected in the original code, rather 
than the autograder copy.
...
with open(AUTOGRADER_FILE) as json_file:
    ...
    for i in data['tests']:
        ...
        if setupBashCommand!="":
            setupProcess = subprocess.Popen(setupBashCommand.split(), 
stdout=subprocess.PIPE,cwd=path)
            try:
                output, error = 
setupProcess.communicate(timeout=AUTOGRADER_TIMEOUT)
            ...
            try:
                ...
                if inputFileConents!="": # appears to be input
                    ...
                    with open(inputFilePath, "r") as inputFile:
                        testProcess = 
subprocess.run(testCommand.split(),encoding='utf-
8',stdin=inputFile,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,cwd=path,timeout
=AUTOGRADER_TIMEOUT)
                    ...
                    try:
                        ...
                        testProcess = 
subprocess.run(testCommand.split(),encoding='utf-
8',stdout=subprocess.PIPE,stderr=subprocess.STDOUT,cwd=path,timeout=AUTOGRADER_TIME
OUT)
                    except OSError: # might be attempting to run a malformed .sh 
file -- correct it (autograder is OK with it)
                        firstCommand = path+testCommand.split()[0]
                        # prepend the corrected script command to the file 
referenced by firstCommand
...
OUT)
                        ...
                        testProcess = 
subprocess.run(testCommand.split(),shell=True,encoding='utf-
8',stdout=subprocess.PIPE,stderr=subprocess.STDOUT,cwd=path,timeout=AUTOGRADER_TIME
OUT)
            except subprocess.TimeoutExpired:
            ...
            if testRan:
                ...
                if regexMode:
                    ...
                    # (regexMode and re.compile(expectedLine).search(outputLine)) 
or
                    # reassemble the output, line by line, into a string with 
newlines in it
                    # check the big string against the regex search! --> success 
yay, fail nooo
                    with open(outputFilePath, 'r') as outputFile:
                        with open(expectedOutputFilePath, 'r') as 
expectedOutputFile:
                            expectedLine = ""
                            for line in expectedOutputFile:
                                expectedLine+=line # probably don't add extra 
newlines to this...
                            outputFileInline = ""
                            for line in outputFile:
                                outputFileInline+=line
                                outputFileInline+="\n"
                            if (re.compile(expectedLine).search(outputFileInline) !
= None):
...
expectedOutputFile:
                            ...
                            while matching: # start evaluating all lines
                                if (expectedLine in outputLine): # start matching 
lines -- THEY MUST BE CONSECUTIVE!
                                    linesMatched += 1
                                    while True:
                                        expectedLine = 
expectedOutputFile.readline()
                                        linesToMatch += 1
                                        outputLine = outputFile.readline()
                                        if not expectedLine: # no more valid lines 
-- we matched them all, so success!
                                            linesToMatch -= 1
                                            successfulTestList.append(testBaseName)
                                            matching = False
                                            print("SUCCESS!")
                                            break
                                        else: # we have another line to try to 
match, check it
                                            if (expectedLine in outputLine): # 
match! great! keep going...
                                                linesMatched += 1
                                                ...
                                                matching = False
                                                break      
                                else: # haven't found it yet, keep looking through 
the output
                                    outputLine = outputFile.readline()
                                    if not outputLine: # we got to the end of the 
output, but didn't match it! 
                                        failedTestList.append(testBaseName)
                                        print("FAILED.")
                                        matching = False
                    print("Test matched: ",linesMatched," of ",linesToMatch," found
...
if (len(failedSetupTestList) > 0):
    print("  TESTS COULD NOT BE SETUP: ",len(failedSetupTestList),": ❌
",failedSetupTestList)
    print("These tests could not be setup for some reason. Most likely, the program
couldn't be compiled.")
    print("")
if (len(erroredTestList) > 0):
    print("  TESTS NOT RUN: ",len(erroredTestList),": ",erroredTestList)❔
    print("These tests could not be run for some reason. Likely need to be checked 
by autograder.")
    print("")
if (len(timedoutTestList) > 0):
    print("  TESTS TIMED OUT: ",len(timedoutTestList),": ",timedoutTestList)⏰
    print("  These tests took too long to run (more ❗️❌❗️
than",AUTOGRADER_TIMEOUT,"). DO NOT COMMIT YET! Look for an infinite loop.  ")❗️❌❗️
    print("")
